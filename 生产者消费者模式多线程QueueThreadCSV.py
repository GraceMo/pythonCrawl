...
class BSSpider(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            for div in divs:
                jokes = div.xpath(".//text()")
                joke = "\n".join(jokes).strip()
                link = self.base_domain + div.xpath(".//a/@href")[0]
...
class BSWriter(threading.Thread):
    ...
    def __init__(self, joke_queue, writer, gLock):
        super(BSWriter, self).__init__()
        self.joke_queue = joke_queue
        self.writer = writer
        self.lock = gLock
